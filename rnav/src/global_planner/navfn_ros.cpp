...
namespace navfn {
  ...
  NavfnROS::NavfnROS(std::string name, navMap::navMap* costmap_ros)
    : costmap_(NULL),  planner_(), initialized_(false), allow_unknown_(true) {
      //initialize the planner
      initialize(name, costmap_ros);
  }

  NavfnROS::NavfnROS(std::string name, navMap::navMap* costmap, std::string global_frame)
    : costmap_(NULL),  planner_(), initialized_(false), allow_unknown_(true) {
      //initialize the planner
      initialize(name, costmap, global_frame);
  }

  void NavfnROS::initialize(std::string name, navMap::navMap* costmap, std::string global_frame){
    if(!initialized_){
      costmap_ = costmap;
   ...
   ROS_WARN("GLOBAL PLANNER INITIALIZED");
  }

  void NavfnROS::initialize(std::string name, navMap::navMap* costmap_ros){
    initialize(name, costmap_ros, "map");
  }
  ...
  void NavfnROS::clearRobotCell(const tf::Stamped<tf::Pose>& global_pose, unsigned int mx, unsigned int my){
    ...
    costmap_->setCost(mx, my, navMap::FREE_SPACE);
  }
