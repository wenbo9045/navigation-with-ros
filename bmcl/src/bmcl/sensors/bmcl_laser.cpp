...
///////////////////////////////////////////////////////////////////////////
//
// Desc: BMCL laser routines
// Author: Andrew Howard
// Date: 6 Feb 2003
// CVS: $Id: bmcl_laser.cc 7057 2008-10-02 00:44:06Z gbiggs $
//
///////////////////////////////////////////////////////////////////////////
#include <algorithm>  
...
#include <unistd.h>
#include <iostream>
#include "bmcl/sensors/bmcl_laser.h"
#include <vector>
#include <map>
#include <cstdlib>
...
////////////////////////////////////////////////////////////////////////////////
// Default constructor
BMCLLaser::BMCLLaser(size_t max_beams, map_t* map) : BMCLSensor(), 
...
void 
BMCLLaser::SetModelBeam(double z_hit,
                        double z_short,
                        double z_max,
                        double z_rand,
                        double sigma_hit,
...
void 
BMCLLaser::SetModelLikelihoodField(double z_hit,
                                   double z_rand,
                                   double sigma_hit,
                                   double max_occ_dist)
{
...
void 
BMCLLaser::SetModelLikelihoodFieldProb(double z_hit,
				       double z_rand,
				       double sigma_hit,
				       double max_occ_dist,
				       bool do_beamskip,
...
bool BMCLLaser::UpdateSensor(pf_t *pf, BMCLSensorData *data)
{
  if (this->max_beams < 2)
    return false;
...
double BMCLLaser::BeamModel(BMCLLaserData *data, pf_sample_set_t* set)
{
  BMCLLaser *self;
  int i, j, step;
  double z, pz;
...
{
 public:
 sampleSet(pf_sample_t *sample) {a = sample;}
 bool operator<(const pf_sample_t* b)
 {
...
bool operator<(const pf_sample_t &a,const pf_sample_t &b)
{
//return (a->weight>=b->weight);
return true;
}
...
double BMCLLaser::LikelihoodFieldModel(BMCLLaserData *data, pf_sample_set_t* set)
{
  BMCLLaser *self;
  int i, j, step;
  double z, pz;
  ...
  double total_weight;
  pf_sample_t *sample;
  pf_sample_t *sample_best;
  pf_vector_t pose;
  pf_vector_t hit;
  int max_weight = 0;
  multimap<double,pf_sample_t*> sampleM;
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      pz += 0.1*self->z_rand * z_rand_mult;
      ...
      // here we have an ad-hoc weighting scheme for combining beam probs
      // works well, though...
      p += pz*pz*pz*pz*pz*pz*pz*1000;
    }

    sample->weight *= p;
    sampleM.insert(make_pair(sample->weight,sample));
  ...
  double bestpose[3]={0,0,0};

  multimap<double,pf_sample_t*>::iterator map_it = sampleM.end();
  multimap<double,pf_sample_t*> sampleX;
  --map_it;
  double weight = map_it->second->weight;
//将较高权值的样本放入x位姿的集合内。
  //multimap<double,pf_sample_t*>::iterator weightIter = sampleX.upper_bound(0.85*weight);
  //sampleX.insert(weightIter,sampleM.end());
  while(map_it!=sampleM.begin())
  {
   if(map_it->second->weight>0.95*weight)
   {sampleX.insert(make_pair(weight,map_it->second));
   --map_it;}
   else 
   break;
  }
  cout<<"num: "<<sampleX.size()<<endl;
 /* double weightT = 0;
  for(multimap<double,pf_sample_t*>::const_iterator iter = sampleX.begin(); iter!=sampleX.end(); iter++)
  {
   weightT += iter->first;
   bestpose[0] += iter->second->pose.v[0]*iter->first;
   bestpose[1] += iter->second->pose.v[1]*iter->first;
   bestpose[2] += iter->second->pose.v[2]*iter->first;
  }
  bestpose[0] /= weightT;
  bestpose[1] /= weightT;
  bestpose[2] /= weightT;
  cout<<"best pose: ("<<bestpose[0]<<" "<<bestpose[1]<<" "<<bestpose[2]<<")"<<endl;
  cout<<endl;*/

  int purification_count = 0;
  int res = set->sample_count/sampleX.size();
  multimap<double,pf_sample_t*>::const_iterator purification_it = sampleX.begin();
  while(purification_count<set->sample_count)
  {
   sample = set->samples + purification_count;
   sample->pose = purification_it->second->pose;
   purification_count++;
   if(purification_count%res==0)
   ++purification_it;
  }
...
double BMCLLaser::LikelihoodFieldModelProb(BMCLLaserData *data, pf_sample_set_t* set)
{
  BMCLLaser *self;
  int i, j, step;
  double z, pz;
...
void BMCLLaser::reallocTempData(int new_max_samples, int new_max_obs){
  if(temp_obs){
    for(int k=0; k < max_samples; k++){
      delete [] temp_obs[k];
    }
