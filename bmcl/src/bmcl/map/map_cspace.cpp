...
class CellData
{
  public:
    ...
    unsigned int src_property_value_;
};
...
void enqueue_line(map_t* map, unsigned int i, unsigned int j, 
	     unsigned int src_i, unsigned int src_j,
	     std::priority_queue<CellData>& Q,
	     CachedDistanceMap* cdm,
	     unsigned char* marked,unsigned int property_value)
{
  if(marked[MAP_INDEX(map, i, j)])
    return;
  ...
  map->cells[MAP_INDEX(map, i, j)].line_occ_dist = distance * map->scale;
  map->cells[MAP_INDEX(map, i, j)].property_value = property_value;
  CellData cell;
  cell.map_ = map;
  cell.i_ = i;
  cell.j_ = j;
  cell.src_i_ = src_i;
  cell.src_j_ = src_j;
  cell.src_property_value_ = property_value;
  Q.push(cell);

  marked[MAP_INDEX(map, i, j)] = 1;
}

// Update the line cspace distance values and property
void line_update_cpace(map_t *map,double max_line_occ_dist)
{
  ...
  CachedDistanceMap* cdm = get_distance_map(map->scale, max_line_occ_dist);
  CellData cell;
  cell.map_ = map;
  for(int i=0; i<map->size_x; i++)
  {
    ...
    {
      if(map->cells[MAP_INDEX(map, i, j)].line_occ_state == +1)
      {
	map->cells[MAP_INDEX(map, i, j)].line_occ_dist = 0.0;
	cell.src_j_ = cell.j_ = j;
	cell.src_property_value_ = map->cells[MAP_INDEX(map, i, j)].property_value;
	marked[MAP_INDEX(map, i, j)] = 1;
	Q.push(cell);
      }
      else
	map->cells[MAP_INDEX(map, i, j)].line_occ_dist = max_line_occ_dist;
    }
  }
  while(!Q.empty())
  {
    ...
    if(current_cell.i_ > 0)
      enqueue_line(map, current_cell.i_-1, current_cell.j_, 
	      current_cell.src_i_, current_cell.src_j_,
	      Q, cdm, marked,current_cell.src_property_value_);
    if(current_cell.j_ > 0)
      enqueue_line(map, current_cell.i_, current_cell.j_-1, 
	      current_cell.src_i_, current_cell.src_j_,
	      Q, cdm, marked,current_cell.src_property_value_);
